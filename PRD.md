...
### 10.2 권장 방안: Decap CMS 자체 호스팅

| 항목 | 내용 |
|---|---|
| 도구 | **Decap CMS** (구 Netlify CMS, 오픈소스, 무료) |
| 배포 위치 | `https://skkustem.org/admin` (Astro `public/admin/index.html` + `config.yml`) |
| 인증 | GitHub OAuth (Cloudflare Workers를 OAuth 프록시로 사용) |
| 백엔드 | GitHub API — 변경 시 commit 생성 → Pages 자동 빌드 → 1~3분 내 사이트 반영 |
| 비용 | 무료 (Cloudflare Workers 무료 티어, 월 100k 요청) |
...
1. ✅ (오늘) 사이트 라이브 + GSC 등록
2. ⬜ `src/content/config.ts` 작성 (zod 스키마 4개)
3. ⬜ TS 데이터 → Markdown frontmatter 변환 스크립트 (Node.js)
4. ⬜ 각 페이지를 Content Collections API로 재연결 (`getCollection('publications')`)
5. ⬜ GitHub OAuth App 생성 (`https://github.com/settings/applications/new`)
6. ⬜ Cloudflare Workers OAuth 프록시 배포 (Decap 공식 템플릿 활용)
7. ⬜ `public/admin/index.html` + `public/admin/config.yml` 작성
8. ⬜ 빌드 + 배포 → `/admin` 접속 → GitHub 인증 → 편집 테스트
