...
- **증상은 계산식이 아니라 배포 주기.** 9월 9일인데 기수가 안 올라간 이유는 값이 `new Date()`로 **빌드 시점에 한 번 굳기** 때문. 마지막 커밋/배포가 2026-08-26이라 HTML에 8월 기준 값(`1·3·6·7·11기`)이 그대로 박혀 있었다. 정적 사이트에서 날짜 파생 값을 쓰면 "코드는 맞는데 화면은 틀린" 상태가 조용히 몇 달 간다.
- **그래서 자동화 지점은 코드가 아니라 재배포다.** `.github/workflows/scheduled-rebuild.yml`이 매월 1일 Cloudflare Pages 배포 훅을 친다. 3월·9월만 걸 수도 있었지만 **매월**로 잡았다 — GitHub 예약 실행은 부하 시 지연·유실될 수 있어 6개월에 한 번뿐인 트리거는 한 번 놓치면 반년을 잃는다. 연차 배지가 1월 1일에 바뀌는 것도 매월 실행이면 자동으로 커버된다.
- **계산식도 규칙대로 다시 썼다.** 기존 `floor(입학 후 개월수/6)+1`은 개인 입학월 기준 6개월 주기라, 3월/9월 입학자에게만 우연히 맞는 식이었다. `2025-04` 입학이 CMS로 들어오면 10월에 기수가 오르는 식으로 어긋난다. 새 `cohortTerm`은 **절대 학기 번호 차이**(3~8월=그 해 1학기, 9~익년 2월=그 해 2학기)로 계산해 입학월과 무관하게 3월·9월에만 오른다. 1~2월을 직전 해 2학기로 묶는 게 핵심 — 12월과 1월이 같은 학기여야 겨울에 기수가 튀지 않는다.
- **KST 보정이 필요한 이유.** Cloudflare/GitHub 빌드 러너는 UTC다. 3월 1일 00:00 KST는 UTC로 2월 28일 15:00이라, UTC 그대로 판정하면 학기 첫날 9시간 동안 이전 학기로 나온다. `nowInKst()`가 +9h를 더한 뒤 UTC getter로 읽어 KST 달력 날짜를 얻는다.
- **남은 선행 조건.** 워크플로는 `CLOUDFLARE_PAGES_DEPLOY_HOOK` 시크릿이 있어야 동작한다. 없으면 `::error::`로 즉시 실패하게 했다 — 빈 URL로 curl이 성공한 척 넘어가면 이번 건과 똑같이 조용히 죽는다.
- **손대지 않은 것 = 연차(`yearsEnrolled`).** 여전히 달력 연도 차이라 1월 1일에 오른다. 학사 기준으로는 3월에 올라야 맞지만 이번 요청 범위가 아니라 그대로 뒀다. 바꾸려면 `academicTerm.ts`에 학년도 계산을 추가하면 된다.
- 검증: 경계 단위 테스트 13건 PASS(scratchpad 일회성), `npm run check` 0/0/0, `typecheck`/`build` 통과, dist 기수 `1·2·4·7·8·12기`.
...
- **연차도 학년도 기준으로 전환(추가 요청).** 기존 `yearsEnrolled`는 달력 연도 차이라 1월 1일에 올랐다. 2025년 3월 입학생이 2026년 1월에 "2년차"가 되는데, 학사상 그 시점은 아직 2025학년도 2학기라 1년차가 맞다. `academicYear()`(3~12월은 그 해, 1~2월은 전년도)를 두고 그 차이로 계산하도록 바꿔 3월에만 오르게 했다. 기수의 학기 경계 규칙과 같은 축을 쓰므로 두 배지가 어긋나지 않는다.
- **오늘 기준으로는 화면이 안 바뀐다.** 9월에는 달력 연도 차이와 학년도 차이가 같은 값을 내서 dist가 `1·2·4·6년차`로 이전과 동일하다. 실제 차이는 1~2월에만 드러나므로, 이 변경의 검증은 화면이 아니라 경계 단위 테스트로만 가능하다.
- 검증: 연차·기수 합동 경계 13건 PASS(2025-03 입학이 2026-01·02에 1년차 유지, 2026-03에 2년차. 2023-09 입학이 2024-01에 1년차 1기, 2024-03에 2년차 2기). 테스트 작성 중 2023-09 입학자의 기대 기수를 2기로 잘못 적어 FAIL이 났는데, 코드가 아니라 기대값이 틀렸다 — 2024-01은 아직 입학 학기(2023학년도 2학기)라 1기가 맞다.
